using ABAPNet.Cluster.Attributes;

namespace ABAPNet.Cluster.Test.Unit
{
    public class Read_Offset
    {
        [Fact]
        public void Read_Struct_With_Offset()
        {
            DataBuffer dataBuffer = new DataBuffer();

            var structExpected = new Cluster()
            {
                Field = new Structure
                {
                    Field1 = ' ',
                    Table1 = new int[] { }
                }
            };

            var structActual = dataBuffer.Import<Cluster>(new byte[] {
                
            });

            Assert.Equal(structExpected.Field.Field1, structActual.Field.Field1);
            Assert.Equal(structExpected.Field.Struct1.Date, structActual.Field.Struct1.Date);
            Assert.Equal(structExpected.Field.Struct1.Time, structActual.Field.Struct1.Time);
            Assert.Equal(structExpected.Field.Table1.Length, structActual.Field.Table1.Length);
        }

        private class Cluster
        {
            [ClusterFieldName("STRUCT")]
            [DeepStruct]
            public Structure Field { get; set; }
        }

        private struct Structure
        {
            [Char(1)]
            public char Field1 { get; set; }

            [FlatStruct]
            public DateTimeStruct Struct1 { get; set; }

            [Int4Table]
            public int[] Table1 { get; set; }
        }

        private struct DateTimeStruct
        {
            [Dats]
            public DateOnly Date { get; set; }

            [Tims]
            public TimeOnly Time { get; set; }
        }
    }
}
