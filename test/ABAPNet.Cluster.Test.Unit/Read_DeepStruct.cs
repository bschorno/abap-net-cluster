using ABAPNet.Cluster.Attributes;

namespace ABAPNet.Cluster.Test.Unit
{
    public class Read_DeepStruct
    {
        [Fact]
        public void Write_Deep_Structure()
        {
            DataBuffer dataBuffer = new DataBuffer();

            var structExpected = new ClusterWithDeepStruct()
            {
                Field = new DeepStruct()
                {
                    Field1 = "AAAA",
                    Field2 = "BBBB",
                    Field3 = new DeeperStruct()
                    {
                        Field1 = 0x01,
                        Field2 = 1
                    }
                }
            };

            var structActual = dataBuffer.Import<ClusterWithDeepStruct>(new byte[] {
                
            });

            Assert.Equal(structExpected.Field.Field1, structActual.Field.Field1);
            Assert.Equal(structExpected.Field.Field2, structActual.Field.Field2);
            Assert.Equal(structExpected.Field.Field3.Field1, structActual.Field.Field3.Field1);
            Assert.Equal(structExpected.Field.Field3.Field2, structActual.Field.Field3.Field2);
        }

        private struct ClusterWithDeepStruct
        {
            [ClusterFieldName("STRUCT")]
            [DeepStruct]
            public DeepStruct Field { get; set; }
        }

        private struct DeepStruct
        {
            [Char(4)]
            public string Field1 { get; set; }

            [String]
            public string Field2 { get; set; }

            [FlatStruct]
            public DeeperStruct Field3 { get; set; }
        }

        private struct DeeperStruct
        {
            [Raw(1)]
            public byte Field1 { get; set; }

            [Int4]
            public int Field2 { get; set; }
        }
    }
}
